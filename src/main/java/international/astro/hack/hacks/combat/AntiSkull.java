...
@Hack.Construct(name = "AntiSkull", description = "rapes skull burrow", category = Hack.Category.COMBAT)
public class AntiSkull extends Hack {
    ...
    public void onTick(TickEvent e){
        ...
        List<BlockPos> strinkList = new ArrayList<>();
        for(BlockPos pos : bubble){

            if (mc.world.getBlockState(pos).getBlock() instanceof BlockSkull) {
                strinkList.add(pos);
            }
        }
        for(BlockPos strink : strinkList){
            int tool = PlayerUtil.getBestAvailableToolSlot(mc.player.world.getBlockState(strink));
            if(silent.isEnabled()){
            ...
            mc.playerController.onPlayerDamageBlock(strink, PlayerUtil.getRayTraceFacing(strink));
